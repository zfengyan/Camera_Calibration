...
namespace easy3d {
    ...
    bool inverse(const Matrix &A, Matrix &invA) {
        ...
        const int n = A.cols();
        if (m != n) {
            std::cerr << "could not compute inverse (not a square matrix)" << std::endl;
            return false;
        }
    ...
    bool solve_least_squares(const Matrix &A, const std::vector<double> &b, std::vector<double> &x) {
        ...
        const int n = A.cols();
        if (n > m) {
            std::cerr << "could not solve: A.cols() > A.rows()" << std::endl;
            return false;
        }

        if (m != b.size()) {
            std::cerr << "could not solve: sizes of A and b don't match" << std::endl;
            return false;
        }
